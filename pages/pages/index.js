...
export default function Home() {
  ...
  const handleGenerate = async () => {
    ...
    for (const type of selectedTypes) {
      ...
      try {
        // Trim to 8000 chars on frontend too so we never send huge payloads
        const trimmedText = bookText.slice(0, 8000);
        const res = await fetch("/api/summarize", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ bookText: trimmedText, summaryType: type }),
        });
